...
/data/init/caldata.txtというファイルにタッチパネルのキャリブレーションデータが書き込まれますが、

デフォルトではデータがないので、立ち上げ時に表示される4角をタッチしてキャリブレーションをしてください。

もう一度キャリブレーションをやり直したい場合は、

/data/init/caldata.txtを開くと、txtファイルを開く前に、4角をタッチしてキャリブレーションする画面が出ます。
...
/data/init/firstAppName.txtに立ち上げたいゲームのパスを一行で書いておくと、それが優先して立ち上がります。
...
64×64のpng画像から任意の大きさをspr()で切り出して使います。
